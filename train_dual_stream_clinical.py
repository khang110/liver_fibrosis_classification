"""Training script for E1/E2: Dual-stream CNN (B-mode + Nakagami) + Clinical Fusion.

This script implements 5-fold cross-validation training for the dual-stream + clinical
fusion models (E1: mean pooling, E2: attention pooling) with early stopping
based on validation AUC.

Usage:
    python train_dual_stream_clinical.py --pooling mean      # E1: Mean pooling
    python train_dual_stream_clinical.py --pooling attention # E2: Attention pooling
"""
...
from models.dual_stream_models import create_dual_stream_clinical_fusion_model
...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler(f"logs/{datetime.now().strftime('%Y%m%d_%H%M%S')}_train_dual_stream_clinical.log", encoding="utf-8"),  # save to file
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train dual-stream + clinical fusion models (E1: mean pooling, E2: attention pooling)"
    )
    parser.add_argument(
        '--pooling',
        type=str,
        ...
        default='mean',
        help='Pooling method: "mean" for E1 or "attention" for E2. Default: mean'
    )
    parser.add_argument(
        '--backbone_bmode',
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help='Dropout probability in fusion MLP. Default: 0.3'
    )
    parser.add_argument(
        '--seed',
        type=int,
    ...
    parser.add_argument(
        '--log_dir',
        type=str,
        default='runs/dual_stream_clinical',
        help='Root directory for TensorBoard logs. Default: runs/dual_stream_clinical'
    )
...
def get_config(args):
    ...
    config = {
        'clinical_csv': Path("data/annotations/175_clinical_8_variables.csv"),
...
def train_epoch(
    ...
    for bmode_imgs, nakagami_imgs, labels, patient_ids in dataloader:
        ...
        # Forward pass
        optimizer.zero_grad()
        logits = model(bmode_imgs, nakagami_imgs, clinical_features)  # (B, 2)
...
def evaluate(
    ...
    with torch.no_grad():
        for bmode_imgs, nakagami_imgs, labels, patient_ids in dataloader:
            ...
            # Forward pass
            logits = model(bmode_imgs, nakagami_imgs, clinical_features)
            loss = criterion(logits, labels)
...
def train(
    ...
    _, backbone_feature_dim = load_backbone(config['backbone_bmode'], config['pretrained'])
    logger.info(f"Backbone feature dimension: {backbone_feature_dim}")
    
    model = create_dual_stream_clinical_fusion_model(
    ...
    logger.info(
        f"Created {config['pooling']} pooling model with B-mode backbone {config['backbone_bmode']} "
    ...
    if best_model_state is not None:
        ...
        logger.info(
            f"Final Val Results (Best Model) at epoch {best_epoch} - "
...
def main():
    ...
    model_name = f"E1 (Mean Pooling)" if config['pooling'] == 'mean' else f"E2 (Attention Pooling)"
