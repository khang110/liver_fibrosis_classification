...
from data.datasets import BModePatientDataset, get_eval_transform, get_train_transform, get_train_transform_advanced, mixup_data, mixup_criterion
...
logging.basicConfig(
    ...
    format='%(message)s',
    handlers=[
        logging.FileHandler(f"logs/{datetime.now().strftime('%Y%m%d_%H%M%S')}_train_clinical.log", encoding="utf-8"),  # save to file
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help='Weight decay (L2 regularization) for optimizer. Default: 0.0 (no weight decay)'
    )
    parser.add_argument(
        '--label_smoothing',
        type=float,
        default=0.0,
        help='Label smoothing factor for CrossEntropyLoss. Range: [0, 1]. Default: 0.0 (no smoothing)'
    )
    parser.add_argument(
        '--log_dir',
        type=str,
...
def get_config(args):
    ...
    config = {
        ...
        'label_column': "CL_F2",
        'image_pattern': "Nakagami_image_*.png",
        'n_folds': 5,
...
def train_epoch(
    ...
    for imgs, labels, patient_ids in dataloader:
        ...
        # Forward pass
        optimizer.zero_grad()
        logits = model(imgs, clinical_features)  # (B, 2)
...
def train(
    ...
    train_dataset = BModePatientDataset(
        train_records,
        transform=get_train_transform_advanced()
    )
    ...
    # Loss and optimizer (multi-class cross-entropy)
    # Add label smoothing for regularization if specified
    ...
    if config['weight_decay'] > 0:
        logger.info(f"Weight decay (L2 reg): {config['weight_decay']}")
    ...
    for epoch in range(config['num_epochs']):
        ...
        # Use a moving average of validation AUC to reduce noise from small validation sets
        # This is especially important in cross-validation where validation sets are small
        if val_auc > best_val_auc + config['early_stopping_min_delta']:
            ...
            patience_counter += 1
            # In CV mode with small validation sets, be more aggressive with early stopping
            # to prevent overfitting when validation metrics are noisy
            effective_patience = config['early_stopping_patience']
            if not config['no_cv'] and len(val_records) < 50:
                # Reduce patience by 2 epochs for small validation sets in CV
                effective_patience = max(8, config['early_stopping_patience'] - 2)
    ...
    if best_model_state is not None:
        ...
        train_loss, train_acc, train_auc, train_confusion, train_threshold_metrics = evaluate(
            model, train_loader, criterion, device, train_clinical_dict, return_confusion=True
        )
...
def main():
    ...
    if config['no_cv']:
        ...
        for fold in range(config['n_folds']):
            ...
            # This ensures each fold starts with the same random state for model initialization
            # The data split is already deterministic via StratifiedKFold with fixed seed
            set_global_seed(config['seed'])
            ...
            val_labels_dist = np.bincount([r.label_binary for r in val_records])
            logger.info(f"Fold {fold + 1} class distribution:")
            logger.info(f"  Train: Class 0={train_labels_dist[0]} ({train_labels_dist[0]/len(train_records)*100:.1f}%), "
                       f"Class 1={train_labels_dist[1]} ({train_labels_dist[1]/len(train_records)*100:.1f}%)")
            logger.info(f"  Val:   Class 0={val_labels_dist[0]} ({val_labels_dist[0]/len(val_records)*100:.1f}%), "
                       f"Class 1={val_labels_dist[1]} ({val_labels_dist[1]/len(val_records)*100:.1f}%)")
